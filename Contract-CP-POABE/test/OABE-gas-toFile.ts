...
describe("Gas Test. For each function.", async () => {
    ...
    let gasUsed = {
        ...
        RegisterDCS:            new Array(attr_nums.length),
        UnregisterDCS:          new Array(attr_nums.length),
        CreateTask:             new Array(attr_nums.length),
        SubmitResult:           new Array(attr_nums.length),
        Challgenge:             new Array(attr_nums.length),
        Prove:                  new Array(attr_nums.length),
        ClaimTaskReward:        new Array(attr_nums.length),
        ClaimChallengeReward:   new Array(attr_nums.length)
    };   // 写成json格式，方便后续处理。RegisterDCS[i]就是属性数量为attrNums[i]时的registerDCS的gas消耗

    // let taskIDs = new Array(4);    // 4个task，1个happy case，3个challenge case
    ...
    async function depolyContract(attrNum: number) {
        // 部署ZKP verifier合约
        let plonk_verifier_path = test_data_path + `attr${attrNum}/evm_verifier.bin`;
    ...
    async function registerDCS(attrIndex: number) {
        ...
        // let attr_num = attr_nums[attrIndex];
        gasUsed.RegisterDCS[attrIndex] = receipt.gasUsed.toNumber();
    ...
    async function createTask(attrIndex: number, taskIds: any[]) {
        let avgGasUsed: BigNumber = BigNumber.from(0);
        for (let i = 0; i < taskIds.length; i++) {
            ...
            // console.log("BigNumber.from(receipt.gasUsed): ", BigNumber.from(receipt.gasUsed));
            avgGasUsed = avgGasUsed.add(BigNumber.from(receipt.gasUsed));
            // let funcName = "createTask-" + (i+1).toString();
            // gasUsed[attrIndex].set(funcName, receipt.gasUsed);
            // console.log(`[${funcName}] Gas Used: `, receipt.gasUsed);
        }
        console.log("avgGasUsed: ", avgGasUsed);
        avgGasUsed = avgGasUsed.div(BigNumber.from(taskIds.length));
        gasUsed.CreateTask[attrIndex] = avgGasUsed.toNumber();
    }
    ...
    async function submitResult(attrIndex: number, taskIds: readonly any[]) {
        ...
        avgGasUsed = avgGasUsed.div(BigNumber.from(taskIds.length));
        gasUsed.SubmitResult[attrIndex] = avgGasUsed.toNumber();
    }
    ...
    async function challenge(attrIndex: number, taskIds: readonly any[]) {
        ...
        avgGasUsed = avgGasUsed.div(BigNumber.from(taskIds.length));
        gasUsed.Challgenge[attrIndex] = avgGasUsed.toNumber();
    }

    async function prove(attrIndex: number, taskIds: readonly any[], toProveWrongTaskIndex: number, toProveRightTaskIndex: number) {
        let avgGasUsed: BigNumber = BigNumber.from(0);
        avgGasUsed = avgGasUsed.add(await proveWrong(attrIndex, taskIds, toProveWrongTaskIndex));
        avgGasUsed = avgGasUsed.add(await proveRight(attrIndex, taskIds, toProveRightTaskIndex));
        gasUsed.Prove[attrIndex] = avgGasUsed.div(BigNumber.from(2)).toNumber();
    }
    ...
    async function proveWrong(attrIndex: number, taskIds: readonly any[], toProveTaskIndex: number) {
        ...
        const proof = hexlify(fs.readFileSync(test_data_path + `attr${attr_nums[attrIndex]}/proof_batch_agg.data`));
        const instances = fs.readFileSync(test_data_path + `attr${attr_nums[attrIndex]}/pi_batch_agg.data`);
    ...
    async function claimTaskReward(attrIndex: number, taskIds: readonly any[], toClaimTaskIndex: number) {
        ...
        // gasUsed[attrIndex].set(funcName, receipt.gasUsed);
        // console.log(`[${funcName}] Gas Used: `, receipt.gasUsed);
        gasUsed.ClaimTaskReward[attrIndex] = receipt.gasUsed.toNumber();
    }
    ...
    async function claimChallengeReward(attrIndex: number, taskIds: readonly any[], toClaimTaskIndex: number) {
        ...
        // gasUsed[attrIndex].set(funcName, receipt.gasUsed);
        // console.log(`[${funcName}] Gas Used: `, receipt.gasUsed);
        gasUsed.ClaimChallengeReward[attrIndex] = receipt.gasUsed.toNumber();
    }
    ...
    async function unregisterDCS(attrIndex: number) {
        ...
        // gasUsed[attrIndex].set(funcName, receipt.gasUsed);
        // console.log(`[${funcName}] Gas Used: `, receipt.gasUsed);
        gasUsed.UnregisterDCS[attrIndex] = receipt.gasUsed.toNumber();
    }
    ...
    after(() => {
        ...
        try {
            ...
            fs.writeFileSync(test_data_path + "function_gas_used.json", jsonString);
        } catch (err) {
